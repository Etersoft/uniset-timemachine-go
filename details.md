...
- Отправка пока одиночным HTTP-клиентом; при необходимости добавим пул воркеров и распределение датчиков по сегментам.
...
3. **Цикл шагов и управление (`replay.Service`)**
   - Обрабатывает команды `pause/resume/stop/step/seek/apply` через управляющий канал; для `step backward/seek` пересчитывает состояние через `BuildState` и перезапускает стрим с новой точки.
