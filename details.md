...
- Данные для отображения публикуем в SharedMemory через HTTP `/set` (либо `stdout` для отладки); UI слушает WebSocket-стрим `/api/v2/ws/state` для таблицы.
...
4. **Клиент SharedMemory (`internal/sharedmem`)**
   ...
   - Реализации: `HTTPClient` (SharedMemory `/set`, поддерживает таймауты/ретраи/воркеры) и `StdoutClient` (печать для тестов); WebSocket нет.
...
5. **CLI (`cmd/timemachine`)**
   - Парсит параметры (`--db`, `--confile`, `--slist`, `--from`, `--to`, `--step`, `--window`, `--speed`, `--batch-size`, `--output`, `--sm-*`, `--ch-table`, `--config-yaml`, `--http-addr`, `--log-cache`, `--show-range`, `-v`).
...
- Страница `/ui/` содержит кнопки Smoke/Flow; при старте любого теста в статусной панели появляется зелёный мигающий индикатор «Идёт тестирование…».
