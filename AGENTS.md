...
- Не коммитим изменения без явной команды пользователя; перед каждым коммитом спрашиваем согласие.
- При разработке и правке тестов: если тест падает, сначала ищем проблему в рабочем коде, и только если её нет — корректируем тест.
- Если меняли код (в том числе фронт в index.html) и timemachine запущен через docker-compose, перед проверками нужно пересобрать и перезапустить контейнер сервера.
- После любых логически завершённых изменений запускайте соответствующие тесты (Go/Playwright или `make js-tests`), если они есть.
